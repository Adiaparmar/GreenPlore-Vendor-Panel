...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (user && user.userId) {
      setUserId(user.userId); // Correctly set userId from the user object
      console.log("User ID set to:", user.userId);
    } else {
    ...
    fetchDataFromApi("/api/products").then((res) => {
      ...
      const filteredProducts = {
        ...
        products: res.products.filter(
          (product) => product.sellerId === user.userId // Match sellerId with user.userId
        ),
      };
  ...
  const deleteProduct = (id) => {
    deleteData(`/api/products/${id}`).then((res) => {
      ...
      // Refresh product list after deletion while maintaining user filter
      fetchDataFromApi("/api/products").then((res) => {
        if (userId) {
          const filteredProducts = {
            ...res,
    ...
    return (
      <div className="right-content w-100">
        <div className="card shadow border-0 p-3">
          <h5>Please log in to view your products</h5>
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="right-content w-100">
        <div className="card shadow border-0 w-100 flex-row p-4 justify-content-between">
          <h5 className="mb-0 breadhead">My Products</h5>
          <Breadcrumbs aria-label="breadcrumb" className="ml-auto breadcrumbs_">
            ...
            <StyledBreadcrumb label="My Products List" />
          </Breadcrumbs>
        </div>
        ...
        <div className="card shadow border-0 p-3 mt-4">
          <h3 className="hd">My Best Selling Products</h3>
